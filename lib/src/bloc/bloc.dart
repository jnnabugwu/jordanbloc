...
import 'package:jordanbloc/src/bloc/bloc_observer.dart';
import 'package:jordanbloc/src/bloc/change.dart';
import 'package:jordanbloc/src/bloc/transition.dart';
...
typedef EventHandler<Event, State> = FutureOr<void> 
Function(Event event, Emitter<State> emit
);
...
/// Called whenever an [event] is [add]ed to the [Bloc].
  ...
  void on<E extends Event>(
    ...
    final subscription = (transformer ?? _eventTransformer)(
      ...
      (dynamic event){
        ...
        final controller = StreamController<E>.broadcast(sync: true,
  ...
  @protected
  @mustCallSuper
  void onTransistion(Transition<Event, State> transition){
    // ignore: invalid_use_of_protected_member
